...
class ControlBoard(View):
	def __init__(self, controller):
		...
		if self.controller.player.isPaused:
	...
	def check(self, member):
		return self.controller.information[self.controller.now_pos]['player'] == member
	...
	async def play_or_pause_(self, button: Button, interaction: Interaction):
		...
		if self.controller.player.isPaused:
			button.emoji = '⏸️'
			await self.controller.player.resume()
		else:
			button.emoji = '▶️'
			await self.controller.player.pause()
	...
	async def last_(self, button: Button, interaction: Interaction):
		...
		self.controller.use_ui = True
		await self.controller.skip(len(self.controller.tracks) - self.controller.now_pos - 1)
		await interaction.response.defer()
		self.play_or_pause_.emoji = '⏸️'
	...
	async def whisper_(self, button: Button, interaction: Interaction):
		...
		self.controller.vol(self.controller.volume - 10)

		if self.controller.volume == 0:
			button.disabled = True

		if self.controller.volume < 1000:
	...
	async def stop_(self, button: Button, interaction: Interaction):
		...
		node = self.controller.player.node
		await self.controller.player.destroy()
	...
	async def loop_(self, button: Button, interaction: Interaction):
		...
		if self.controller.loop_range is None:
			...
			button.emoji = '🔁'
			self.controller.loop_range = [0, len(self.controller.tracks)-1]
